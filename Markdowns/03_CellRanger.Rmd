...
`gzip` (as indicated by the `.gz` filename suffix; the format that Ensembl
provides them as). Therefore, make sure to decompress the files first if
necessary (you can do this using the `gunzip` command).
...
reference are the "gene_id", "gene_name" and "transcript_id".
...
```{bash, eval=FALSE}
sbatch cellranger_scripts/CellRanger_MakeRef.sh
```
...
We will be a piece of software called "Clarity Tools" to retireve the data from
the servers. We will first need to download the clarity tools file.
...
```
sbatch cellranger_scripts/Fetch_sequence_data.sh SLX-21334
```

This script is different to the `CellRanger_MakeRef.sh` script, in that we have
not "hard coded" the SLX ID into the script, but rather need to include in the
command calling the script. This means we can reuse the script with any
sequencing project.
...
single file. In order to separate them the automated process compares the index
pair (i_1 and i_2) to the list of indexes assigned to each sample (from the
Genomic LIMS) and assigns reads to the appropriate sample fastq. Due to
technical errors, e.g. sequencing errors, there will always be a small number of
reads with indexes that do not match any expected indexes. These reads are
...
The remaining files relate to the automated processing of the raw sequencing data
(in bcl2 format) to the fastq format that we will use:
...
### Rename the fastq to the correct format for CellRanger
...
specific sample. The convention is the default convention used by `bcl2fastq`
...
corresponding fastq file. In the **cellranger_scripts** directory there is a
...
A complete command for processing one of the samples might be:
