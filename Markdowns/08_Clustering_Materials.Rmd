...
We will be using gene symbols to identify the marker genes, so we will switch the
rownames in the SCE object to be gene symbols. We use the scate function
...
```{r}
p1 <- plotTSNE(sce, by_exprs_values = "reconstructed", colour_by = "FCGR3A")
p2 <- plotTSNE(sce, by_exprs_values = "reconstructed", colour_by = "MS4A7")
p1 + p2
```
...
```{r}
p1 <- plotTSNE(sce, by_exprs_values = "reconstructed", colour_by = "FCER1A")
p2 <- plotTSNE(sce, by_exprs_values = "reconstructed", colour_by = "CST3")
p1 + p2
```
