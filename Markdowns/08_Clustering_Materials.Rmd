...
with some of the text copied with a few edits. Also see the OSCA book's [chapter
on clustering](https://osca.bioconductor.org/clustering.html). In particular,
please read the
...
of the analysis depends on the dataset. One of the most promising applications
...
after feature selection and dimensionality reduction, usually on the PCA output.

We will focus graph-based clustering, however, it is also possible to apply
hierachical clustering and k-means clustering on smaller data sets - see the
[OSCA
...
We will use the data set generated in the preivous session. This contains 7
samples from the Caron dataset. For the purposes of these materials, in the
...
cells within the network. A graph-based clustering method has three key parameters:
...
pacakge documentation for the `makeSNNGraph` function):
...
on two methods: walktrap and louvain. See the [OSCA
...
* build the graph and define clusters
...
number of different packages, in particular the *bluster* package. *scran*
provides a handy "wrapper" function `clusterCells` that allows us use a variety
of different algorithms with on simple command.
...
agrument `full = TRUE`. If you are only interested in retrieving the clusters,
...
The number of cells in the data set is large an plotting all the cells would
...
pacakge](https://igraph.org/r/doc/subgraph.html). Cells can be color-coded by
sample type:
...
V(snn.gr)$SampleGroup <- colData(sce)$SampleGroup
...
More commonly we will visualise the clusters by superimposing them on a tSNE or UMAP plot. 
We can store the clusters in the `sce` object `colData`.

```{r}
sce$Clusters1 <- clustering1$clusters
plotReducedDim(sce, colour_by="Clusters1", dimred="TSNE")
```
...
Walktrap is the default algorithm for `clusterCells`, *k* is set
to 10 by default and the default edge weighting is "rank". To explicitly request walktrap and to set the *k* to a different number
of nearest neighbours, we use a `SNNGraphParam` object from the *bluster* 
package (which is the package *clusterCells* is using under the hood). 

Let's set the k to 15 but keep the other parameters the same. This 
time we will just return the clusters:
...
Most clusters comprise cells from several replicates of a same sample type, cluster 6
appears to be predominantly cells from teh ETV6-RUNX samples.
...
plotTSNE(sce, colour_by="louvain15") + facet_wrap(~ sce$SampleGroup)
```
...
This method is not currently implemented in *scran*/*igraph* but is used in
Seurat with some jumping through hoops to make use of some Python packages. To
keep things simple we will not cover it in this course.

# Assessing cluster behaviour

A variety of metrics are avialable to aid us in assessing the behaviour of a 
particular clustering method on our data. These can help us is assessing how
...
cluster is from cells in other cluster, and to compare the results of 
...
maximum of the two values. Cells with a large silhouette are stongly related to
...
We could also look at the correspondence between different clusters by these
numbers on a grid showing for each cluster number of cells in that cluster that
are closer to another cluster, colouring each tile by the proportion of the total
cells in the cluster that it contains. Ideally we would like to see a strong 
diagonal band and only a few off-diagonal tile containing small number of cells.
...
required - perhaps there should more than 8 clusters to adequately reflect the
biology.
...
between each pair of clusters using teh `pairwiseModularity` function from the
...
Largely, this reflects what we saw from the silhouette widths, but also reveals some additional
inter-connectedness between other clusters e.g. cluster 3 and cluster 9. We can also visualise this
as network graph where nodes are clusters and the edge weights are the modularity.
...
```{r}
jacc.mat <- linkClustersMatrix(sce$walktrap15, sce$louvain15)
...
We can see that Louvain clusters 4, 5, and 6 are equivalent to walktrap clusters
12, 8 and 4. The remaining Louvain clusters are combinations of cells from
...
As we have seen, there are a number of different parameters we can change to alter
the final clustering result - the edge weighting method, the *k* and the clustering
algorithm. There is no one gold standard that will fit all data, so, in most cases,
it is necessary to assess a number of different clusterings to obtain one that provides
a view of the data that fits our biological interpretations. The `clusterSweep` function
allows us to apply a range of different parameters in one go and obtain the clustering
for each.
...
The resulting object is list containing a DataFrame with the clusters for each combination of the clustering parameters and
a corresponding DataFrame showing the parameters used to generate each of these:
...
also means the job can be massively more parallelised by using more cores.
We will see this in the exercise.

## Expression of known marker genes

If we expect our clusters to represent know cell types for which there are 
...
sce$walktrap10 <- clusterCells(sce, 
                           use.dimred = "corrected", 
                           BLUSPARAM = SNNGraphParam(k = 10, cluster.fun = "walktrap"))
pTSNE <- plotTSNE(sce, colour_by = "walktrap10") +
  ggtitle("Walktrap clusters")
pTSNE
```

Having identified clusters, we now display the level of expression of cell type
marker genes to quickly match clusters with cell types (e.g. MS4A1 for B cell
marker gene). For each marker we will plot its expression on a t-SNE, and show
...
plotMarkers <- function(ctx, sceObj=sce, genes=markGenesFull) {
  
    genes <- genes[[ctx]]
  
    pTSNE <- plotTSNE(sceObj, colour_by = "walktrap20") +
               ggtitle("Walktrap clusters")
    p1 <- plotTSNE(sceObj, by_exprs_values = "reconstructed", colour_by = genes[1]) +
              ggtitle(genes[1])
    if(length(genes)==2){
        p2 <- plotTSNE(sceObj, by_exprs_values = "reconstructed", colour_by = genes[2]) +
                  ggtitle(genes[2])
    }

    expPlot <- plotExpression(sceObj, 
                   exprs_values = "reconstructed",
                   x = "walktrap20", 
                   colour_by = "walktrap20",
                   features=genes) + 
                 ggtitle(ctx)
    if(length(genes)==1){
        (pTSNE + p1) / expPlot
...
```{r, fig.width=10, fig.height=5}
plotMarkers("Naive CD4+ T cells")
```

## Memory CD4+

```{r, fig.width=10, fig.height=5}
plotMarkers("Memory CD4+ T cells")
```
...
```{r, fig.width=10, fig.height=5}
plotMarkers("CD8+ T cells")
```

## NK cells

```{r, fig.width=10, fig.height=5}
plotMarkers("NK cells")
```

## CD14+ Monocytes

```{r, fig.width=10, fig.height=5}
plotMarkers("CD14+ Monocytes")
```

## FCGR3A+ Monocytes

```{r, fig.width=10, fig.height=5}
plotMarkers("FCGR3A+ Monocytes")
```

## Dendritic Cells

```{r, fig.width=10, fig.height=5}
plotMarkers("Dendritic Cells")
```

## Platelets

```{r, fig.width=10, fig.height=5}
plotMarkers("Platelets")
```
