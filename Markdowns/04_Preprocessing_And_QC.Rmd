...
author: "Stephane Ballereau, Ashley Sawle and Chandra Chilamakuri"
output:
  html_document:
    df_print: paged
    toc: yes
...
* Data set 1: [CaronBourque2020](https://www.nature.com/articles/s41598-020-64929-x): Data from Childhood acute lymphoblastic leukemia (cALL)
  * 12 samples 
  * Sample groups
...
* Data set 2: HCA (Human Cell Atlas) Census of Immune Cells for adult Bone Marrow Mononuclear Cells (BMMCs)
  * ABMMC: 8 replicates
...
* What will you learn from this section? 
  * Types of input files loaded 
  * What is the structure of the 'SingleCellExperiment' R object? 
  * How to filter out genes  
  * How to check the quality of cells and remove bad ones
...
![](../Images/RStudioServer_message.png)
...
* **[DropletUtils](https://bioconductor.org/packages/release/bioc/html/DropletUtils.html)** - utilities for the analysis of droplet-based, inc. cell counting
* **[scater](https://bioconductor.org/packages/release/bioc/html/scater.html)** - QC
* **[ensembldb](https://bioconductor.org/packages/release/bioc/html/ensembldb.html)** & **[AnnotationHub](https://bioconductor.org/packages/release/bioc/html/AnnotationHub.html)** - for gene annotation
* **[BiocParallel](https://bioconductor.org/packages/release/bioc/html/BiocParallel.html)** - for parallelisation of some functions
* **[tidyverse](https://www.tidyverse.org/)**, **[patchwork](https://patchwork.data-imaginist.com/)**  
* **[ggvenn](https://cran.r-project.org/web/packages/ggvenn/index.html)** - for data.frame manipulation and plotting
* **[patchwork](https://patchwork.data-imaginist.com/)**: combining ggplots
* **[DT](https://rstudio.github.io/DT/)**: creating nice HTML tables when rendering this RMarkdown to a html report.
...
```{r samplesheet_preProc, message=TRUE}
samplesheet <- read_tsv("Data/sample_sheet.tsv", show_col_types = FALSE)
```

```{r samplesheet_display_preProc, echo=TRUE}
samplesheet %>%
...
* Note that Barcodes, Cells and Droplets used synonymously.  
* UMI barcode a (DNA/RNA) molecule within the cell, UMIs, transcripts and counts used synonymously.

* filtered_feature_bc_matrix contains three files
  * matrix.mtx.gz: 
    * Number of UMIs for a given gene in a given cell
    * Sparce matrix
  * features.tsv.gz:
    * Gene ID, gene symbol and data type (expression)
  * barcodes.tsv.gz:
    * Cell barcodes present in our data
...
## Metadata for features/genes: how to access them?
...
## Metadata for cells: how to access them?
...
# QC on scRNA-seq raw counts data 

<!-- https://osca.bioconductor.org/quality-control.html -->

**The purpose of QC on raw counts data**: The cell calling performed by CellRanger 
does not always retain only droplets containing cells. Poor-quality cells, or
rather droplets, may be caused by cell damage during dissociation or failed
library preparation. They usually have low UMI counts, few genes detected and/or
high mitochondrial content. The presence of these droplets in the data set may
affect normalisation, assessment of cell population heterogeneity, clustering
and trajectory:
...
* The **number of expressed genes in each cell** defined as the number of genes
  ...
  been successfully captured. But be aware that 
...
* **Cell counts per sample**: Low cell counts may indicate bad quality sample

    
* **Challenges in QC**:
  * Selecting the appropriate filter parameters in order to retain as many high quality cells as possible and remove as many true poor quality cells as possible
  * Distinguish poor quality cells from less complex types of cells. 
  * Differentiate between doublets and complex cell types
...
## Explore QC metrics from single sample
...
```{r properties_distribNbGenesDetected, echo=TRUE}
genesPerCell <- colSums(counts(sce.sing) > 0)
hist(genesPerCell, xlab = 'Genes / cell', ylab='Cell counts')
```

### Genes in reference vs detected genes in cells   

```{r gene_counts_barplots, echo=TRUE }
genes_in_ref = nrow(sce.sing)

bar_tab <- tibble(
  genes_in_ref = nrow(sce.sing),
  exp_atleast_once = sum(rowSums(counts(sce.sing)) > 0),
  max_genes_per_cell = max(genesPerCell),
  min_genes_per_cell = min(genesPerCell)
)

bar_tab <- bar_tab %>% 
  pivot_longer( cols = everything(),names_to =  'bin', values_to = 'counts') %>% 
  mutate( percent= (counts / genes_in_ref ) * 100) %>% 
  mutate(bin = fct_inorder(bin))

p1 <- ggplot(bar_tab, aes(x=bin, y= counts)) +
  geom_bar( stat = 'identity') +
  labs(x='', y='Gene counts') +
  theme_classic() +
  theme( axis.text.x = element_text( angle = 90, vjust = 0.5))
  
p2 <- ggplot(bar_tab, aes(x=bin, y= percent)) +
  geom_bar( stat = 'identity') +
  labs(x='', y='% Gene counts ') +
  theme_classic() +
  theme( axis.text.x = element_text( angle = 90, vjust = 0.5))
...
### Relative UMIs distrubution for a gene across cells

We could also look at the distribution of UMIs for individual genes across all
...
```{r annotate_genes}
ah <- AnnotationHub()
ens.hsa.98 <- query(ah, c("Homo sapiens", "EnsDb", 98))[[1]] 

genes <- rowData(sce)$ID
gene_annot <- AnnotationDbi::select(ens.hsa.98, 
                                    keys = genes,
                                    keytype = "GENEID",
...
* **sum**: total UMI count per cell
* **detected**: number of features (genes) detected per cell
* **subsets_Mito_sum**: number of UMIs mapped to mitochondrial transcripts per cell 
* **subsets_Mito_detected**: number of mitochondrial genes detected per cell
* **subsets_Mito_percent**: percentage of UMIs mapped to mitochondrial transcripts per cell
* **total**: also the total UMI count per cell
...
colData(sce) %>% 
    ...
    ggplot(aes(x = sum, y = detected)) +
      geom_point(aes(colour = subsets_Mito_percent > 10), alpha=0.3) + 
...
Assuming that most of the dataset consists of high-quality cells. Then, we
determine which cells are outliers for each QC metric, based on the [median
absolute deviation
(MAD)](https://en.wikipedia.org/wiki/Median_absolute_deviation) from the median
value of each metric across all cells.  (not the mean and the standard deviation
which both are sensitive to outliers).
...
numbers of cells with very high UMI counts may indicate doublets). We therefore log transform the
...
```{r}
par(mfrow=c(1,2))
hist(sce$sum, xlab='UMI counts / cell', main='Before Log-transformation')
hist(log10(sce$sum + 1), xlab='UMI counts / cell', main='After Log-transformation')
```
...
As with the library size, we will log transform the number of genes detected 
...
plotColData(sce, 
            ...
            other_fields="Sample",
            colour_by="discard") +
    facet_wrap(vars(Sample), ncol=5, scale="free_x")
```

# QC and Filtering based on sparsity

We will here specifically look at the sparsity of the data, both at the gene and cell levels.
