...
on differences between the mean expression level, or the probability of
expression between clusters or take the whole distribution of expression into
account.
...
As always, the [OSCA chapter on marker detection](https://bioconductor.org/books/release/OSCA/marker-detection.html) contains additional detail and advice.
...
* The SCE with logcounts normalised within each batch generated in the [Batch
Correction](07_BatchCorrection.html) section.  
* The SCE with batch-corrected data integration using the `batchelor::fastMNN()`
algorithm, including clusters covered in the [Clustering](08_Clustering.html)
section.  
...
gene names instead of Ensembl IDs. Using the `uniquifyFeatureNames()` function
makes this safely, avoiding duplicate gene names.

As a reminder, the `corrected` object contains the cell cluster assignments
obtained, for example, with the "walktrap" algorithm with *k*=15:
...
colData(uncorrected)$walktrap <- factor(colData(corrected)$walktrap15)
```
...
values, these should not be used for for gene-based analyses like marker gene
detection.
...
In our case, valid assays to use in differential analysis tests are the
...
markers <- scoreMarkers(uncorrected, 
                        groups = factor(uncorrected$walktrap), 
...
which the gene is detected in the cluster of interest versus the number of cells
in which the gene is detected in the other cluster. Positive values indicate
that the gene is detected in a more cells in the cluster of interest than the
other cluster. Note this takes no account of the magnitude of the gene
expression.

More detail on the differences between these effect sizes can be found in the
["Advanced" Marker detection chapter of the OSCA
...
`full.stats=TRUE` to the `scoreMarkers` command, by default score markers
...
* **median.X** - this is the median of the score across all pairwise comparisons.
It is more robust to outliers than the mean. Whilst this is beneficial in
avoiding the situation where the effect size vary large in only a small number
of comparisons, it may conversely be detrimental to marker gene selection if
there are small number of clusters with very similar effect sizes, as these will
...
upregulated genes as a high score indicates the gene is upregulated in the
cluster of interest relative to *at least one* other clusters. Conversely, if
the maximum is low, then the gene is downregulated relative to *all* other
clusters.  
* **rank.X** - This is the minimum ranking of that gene by that score across all
clusters. For each pairiwse comparison the genes are ranked according to the
score, this summary provides the lowest rank for that gene across all
comparisons. Essentiall, a gene with a low "min-rank" will be one of the best
genes according to that score in at least one comparison.
...
generate a list of genes that is the union of genes with a rank of 5 or less.
This will ensure we get at least 5 genes that distinguish the cluster of
interest from all other clusters.

For exmple using the min-rank for Cohen's *d* on cluster 11:
...
We can plot the distribution of  expression of sepcific genes as we have previously, e.g. FCGR3A (CD16), a monocyte marker:
